...
from read_csv('merged-kg_kg-microbe-humanasahost-traits_nodes.tsv', filename=true, union_by_name=true);


create or replace table uniprot_nodes as 
select
    id, name, description, category, xref, provided_by, synonym, object, predicate, relation, same_as, subject, subsets
from read_csv('uniprot_genome_features/merged-kg_uniprot_nodes.tsv', filename=true, union_by_name=true); 
...
create or replace table merged_edges_subset as select * from read_csv('merged-kg_kg-microbe-humanasahost-traits_edges.tsv', filename=true, union_by_name=true);

create or replace table uniprot_edges as select * from read_csv('uniprot_genome_features/merged-kg_uniprot_edges.tsv', filename=true, union_by_name=true);
...
FROM (
    SELECT *,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY id
            -- ORDER BY CASE WHEN filename LIKE 'merged-kg_nodes.tsv' THEN 1 ELSE 2 END
        ) as rn
    FROM merged_nodes_subset
) sub
WHERE sub.rn = 1;
