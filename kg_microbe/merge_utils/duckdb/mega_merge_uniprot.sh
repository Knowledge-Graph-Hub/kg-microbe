...
-- create or replace table raw_nodes as select * from read_csv('merged-kg/merged-kg_nodes.tsv', filename=true, union_by_name=true); 


create or replace table raw_nodes as select * from read_csv('uniprot_genome_features/nodes.tsv', filename=true, union_by_name=true); 
create or replace table raw_edges as select * from read_csv('uniprot_genome_features/edges.tsv', filename=true, union_by_name=true);
...
ExamplesOfDuplicates AS (
    SELECT 
        ...
        rn.synonym, rn.iri, rn.object, rn.predicate, rn.relation,
...
FROM (
    SELECT *,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY id
            ORDER BY CASE WHEN filename LIKE 'merged-kg/merged-kg_nodes.tsv' THEN 1 ELSE 2 END
        ) as rn
    FROM raw_nodes
) sub
WHERE sub.rn = 1;
...
COPY (
    SELECT id, category, name, description, xref, provided_by, synonym, iri, object, predicate, relation, same_as, subject, subsets
    FROM nodes
...
-- get all duplicate edges by subject/object pairs from raw_edges
...
SELECT subject, predicate, object
  FROM raw_edges
  WHERE subject LIKE 'CHEBI:%' AND object LIKE 'UniprotKB:%'
LIMIT 10;
...
COPY (
    ...
    FROM edges
) TO 'merged-kg_uniprot_edges.tsv' WITH (FORMAT 'csv', DELIMITER '\t', HEADER true);
...
-- LIKELY this is wrong because it doesnt remove duplicates 
...
INSERT INTO nodes (id, name, description, category, xref, provided_by, synonym, object, predicate, relation, same_as, subject, subsets)
...
DROP TABLE merged_nodes_subset;

COPY (
    SELECT id, category, name, description, xref, provided_by, synonym, object, predicate, relation, same_as, subject, subsets
    FROM nodes
) TO 'merged-kg_uniprot_kg-microbe-host_subset_nodes.tsv' WITH (FORMAT 'csv', DELIMITER '\t', HEADER true);
...
INSERT INTO edges (subject, predicate, object, relation, primary_knowledge_source)
...
COPY (
    SELECT
        ...
        primary_knowledge_source
    FROM edges
) TO 'merged-kg_uniprot_kg-microbe-host_subset_edges.tsv' WITH (FORMAT 'csv', DELIMITER '\t', HEADER true);


-- Then merge with kg base graph
...
COPY (
    SELECT id, category, name, description, xref, provided_by, synonym, object, predicate, relation, same_as, subject, subsets
    FROM nodes
) TO 'merged-kg_all_nodes.tsv' WITH (FORMAT 'csv', DELIMITER '\t', HEADER true);

-- for edges

create or replace table merged_edges_subset as select * from read_csv('merged-kg_kg-microbe-base/merged-kg_edges.tsv', filename=true, union_by_name=true);
...
COPY (
    SELECT
        ...
        primary_knowledge_source
    FROM edges
) TO 'merged-kg_all_edges.tsv' WITH (FORMAT 'csv', DELIMITER '\t', HEADER true);



-- to merge kg host with base graph
