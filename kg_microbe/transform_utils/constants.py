...
# Columns from madin_etal.csv (added if not already present)
SPECIES_TAX_ID_COLUMN = "species_tax_id"
...
GRAM_STAIN_COLUMN = "gram_stain"
SPORULATION_COLUMN = "sporulation"
MOTILITY_COLUMN = "motility"
RANGE_TMP_COLUMN = "range_tmp"
RANGE_SALINITY_COLUMN = "range_salinity"
D1_LO_COLUMN = "d1_lo"
D1_UP_COLUMN = "d1_up"
D2_LO_COLUMN = "d2_lo"
D2_UP_COLUMN = "d2_up"
DOUBLING_H_COLUMN = "doubling_h"
GENOME_SIZE_COLUMN = "genome_size"
GC_CONTENT_COLUMN = "gc_content"
CODING_GENES_COLUMN = "coding_genes"
OPTIMUM_TMP_COLUMN = "optimum_tmp"
OPTIMUM_PH_COLUMN = "optimum_ph"
GROWTH_TMP_COLUMN = "growth_tmp"
RRNA16S_GENES_COLUMN = "rRNA16S_genes"
TRNA_GENES_COLUMN = "tRNA_genes"
REF_ID_COLUMN = "ref_id"

# Grouped list for Madin et al columns for convenience (uses the column constants above)
MADIN_ETAL_COLUMNS = [
    TAX_ID_COLUMN,
...
def generate_column_constants(columns: list, overwrite: bool = False) -> None:
    ...
    `TAX_ID_COLUMN` with value `'tax_id'`.

    This function mutates the module globals. It's safe to call multiple
    times; by default it will not overwrite existing names unless
    `overwrite=True`.
    """
    g = globals()
    for col in columns:
        const_name = _sanitize_constant_name(col)
        if const_name in g and not overwrite:
            continue
        g[const_name] = col


# Opt-in auto-generation: set this environment variable to '1' to create
