...
def edge_metadata_for(channel: str, evidence_class: str) -> Tuple[str, str]:
    ...
    A citation overrides the channel default: the row combines curated metadata
    with text mining over the linked abstract, which the authors themselves
    score one tier lower.

    Within the genome channel, a habitat value means the association comes from
    sample/isolation metadata rather than the annotation pipeline the channel is
    named for, so ``observation`` describes it and ``knowledge_assertion`` does
    not (#716).

    Note this is **not** a confidence demotion, and the two signals are
    orthogonal: measured over the first 3M rows of the real genome archive, all
    1,693 habitat rows carry score **4** — PREGO's highest tier, not its lowest.
    Biolink's ``knowledge_level`` is an unordered description of *how* the
    knowledge was produced, so a high-confidence observation is coherent. An
    earlier version of this docstring claimed those rows scored 3; that was
    inherited from a review comment and is false.

    The habitat rule is deliberately nested inside the genome-channel branch
    rather than checked before it. Hoisted, it answered for channels this code
    knows nothing about — ``edge_metadata_for("metagenomes", "habitat")``
    returned a confident ``("observation", "automated_agent")`` — which breaks
    the invariant that an unrecognised channel declines to assert provenance.
    ...
    if channel == CHANNEL_GENOMES:
        if evidence_class == EVIDENCE_HABITAT:
            # Where the organism was found, not what its genome encodes.
            return "observation", "automated_agent"
