...
def edge_metadata_for(channel: str, evidence_class: str) -> Tuple[str, str]:
    ...
    Two evidence classes override the channel default.

    A citation means the row combines curated metadata with text mining over
    the linked abstract, which the authors themselves score one tier lower.

    A habitat means the association comes from sample/isolation metadata, not
    from the annotation pipeline the channel is named for. Those rows carry
    score 3 in the real genome archive — PREGO's lower tier, the same one it
    gives text-mined rows — yet without this branch they inherited the genome
    channel's ``knowledge_assertion``, the *highest* tier, on ~756 of a 300k
    sample. ``observation`` is the honest description: a recorded fact about
    where an organism was found, neither an annotation-derived assertion nor a
    computed statistic (#716).
