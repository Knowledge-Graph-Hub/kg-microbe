    ...
    with open(empty_org_file, "w") as e:
        ...
        with tqdm(total=len(organism_ids), desc="Processing files") as progress:
            for i in range(0, len(organism_ids), batch_size):
                values = _get_uniprot_batch_organism(
                    organism_ids,
                    base_url,
                    i,
                    fields,
                ...
                final_organism_id = organism_ids[min(i + batch_size, len(organism_ids)) - 1]
                progress.set_description(
                    f"Downloading organism data from Uniprot, final file of batch: {final_organism_id}.json"
                )
...
def parse_response(res, values):
    ...
    for line in res.iter_lines():
        ...
        if headers is None:
            ...
            res = dict(zip(headers, tokens, strict=False))
            values.append(res)
