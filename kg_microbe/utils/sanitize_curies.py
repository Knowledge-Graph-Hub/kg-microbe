...
def main(nodes, edges, nodes_fixed, edges_fixed):
    ...
    Only specific fields (id, subject, object, predicate, relation, category) are sanitized, and only problematic
    characters (such as >°<[]{}|\\^`") in CURIE suffixes and the path segment of URIs are URL-encoded. The entire
    URI is not encoded; the structure (scheme, netloc) is preserved, and only the path portion is encoded as needed.
    """
