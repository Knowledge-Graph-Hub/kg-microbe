...
The refreshed coverage audit streams all 31 current producer edge tables, including ignored data (18,784,312 rows). The new bundle leaves 485 historical identifiers unreconstructed, versus 476 with the older pinned MIM input. The nine additional IDs have no references in these producer tables: observed exposure remains **504 edge rows referencing 63 IDs**. This is existing graph exposure, not a claim of edge deletion or completed producer rebuild. Full results are in `chemical-scopes-20260923/coverage-current-graph.{json,tsv}`; reproduce with `audit_current_graph.py --data-root <data> --candidate-report <candidate-directory/report.json> --output-directory <audit-directory>`.
