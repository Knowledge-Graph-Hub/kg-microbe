...
def main():
    ...
    for i, ko_id in enumerate(FAILED_KOS, 1):
        ...
        if details:
            ...
            logger.info(
                f"  Created {len(details.get('pathways', []))} pathway edges and {len(details.get('modules', []))} module edges"
            )
        else:
    ...
    if new_edges:
        ...
        with open(RETRY_EDGES_FILE, "w") as f:
            writer = csv.DictWriter(
                f, fieldnames=["subject", "predicate", "object", "relation", "primary_knowledge_source"], delimiter="\t"
            )
            writer.writeheader()
