...
def create_1hop_visualization(nodes_df, edges_df, center_id, output_file):
    ...
    plt.title(
        f"1-Hop Subgraph around {center_name_full}\n({len(G.nodes)} nodes, {len(G.edges)} edges)",
        fontsize=20,
        fontweight="bold",
        pad=40,
    )
    ...
    # Add hop legend
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=18, label="Center (Hop 0)")
    )
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=12, label="1-Hop Neighbors")
    )
...
def main():
    ...
    parser.add_argument(
        "--center-id", type=str, default="NCBITaxon:1718", help="Center node ID (default: NCBITaxon:1718)"
    )
