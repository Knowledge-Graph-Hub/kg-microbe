...
def create_2hop_full_labels():
    ...
    # Add title
    plt.title(
        f"2-Hop Subgraph: {center_name} - FULL 1-HOP LABELS\n({len(G.nodes)} nodes, {len(G.edges)} edges)",
        fontsize=26,
        fontweight="bold",
        pad=60,
    )
    ...
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=15, label="1-Hop (Full Labels)")
    )
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=10, label="2-Hop (Top 20)")
    )
