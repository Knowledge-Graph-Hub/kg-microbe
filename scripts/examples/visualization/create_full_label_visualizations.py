...
def create_1hop_full_labels():
    ...
    for _, node in nodes_df.iterrows():
        G.add_node(node["id"], name=node["name"], category=node["category"], hop=node["hop"])
    ...
    # Draw center node
    center_categories = (
        str(G.nodes[center_id]["category"]).split("|") if G.nodes[center_id]["category"] else ["unknown"]
    )
    ...
    plt.title(
        f"1-Hop Subgraph around {center_name_full} - FULL LABELS\n({len(G.nodes)} nodes, {len(G.edges)} edges)",
        fontsize=24,
        fontweight="bold",
        pad=50,
    )
    ...
    # Add hop legend
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=20, label="Center (Hop 0)")
    )
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=15, label="1-Hop Neighbors")
    )
...
def create_2hop_full_labels():
    ...
    center_name_full = str(G.nodes[center_id]["name"]) if G.nodes[center_id]["name"] else center_id
    plt.title(
        f"2-Hop Subgraph around {center_name_full} - FULL LABELS\n({len(G.nodes)} nodes, {len(G.edges)} edges)",
        fontsize=26,
        fontweight="bold",
        pad=60,
    )
    ...
    legend_elements.append(
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="black", markersize=10, label="2-Hop Neighbors")
    )
