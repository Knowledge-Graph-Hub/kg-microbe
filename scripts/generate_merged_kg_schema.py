...
def build_schema(info, merged_dir):
    ...
    schema = {
        ...
        "description": (
            ...
            f"{info['n_nodes']:,} nodes and {info['n_edges']:,} edges. List-valued node columns "
            "(category, xref, provided_by, synonym, same_as) are '|'-delimited in the TSV. "
            ...
            "literals and raw filenames; unit carries some mojibake; knowledge_level/agent_type "
            "carry a few '|'-joined merge artifacts (excluded from the enums)."
        ),
